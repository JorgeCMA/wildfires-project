...
def fetch_weather_batch(
    ...
    """Fetch weather for multiple locations in a single request.

    Open-Meteo supports comma-separated coordinates for batch requests.

    Parameters
    ----------
    locations:
        List of dicts, each with ``latitude`` and ``longitude`` keys.
    start_date:
        Start date as ``"YYYY-MM-DD"``.
    end_date:
        End date as ``"YYYY-MM-DD"``.
